...
public class Main {
    ...
    public static void main(String[] args){
        //We read csv, transform it into a vector, and pass it to our KNN class
        ...
        try(BufferedReader br = new BufferedReader(new FileReader("drug200.csv"))){
            ...
            while((line = br.readLine()) != null){
                //We take what we read and write it in X
                //we use DELIMITER because readLine won't break what we read in columns
        ...
        //We create the columns and then in each column we will write the values separately
        ...
        for(int i = 0 ; i < size; i++){
            List<Double> doubleList = bps.get(i);

            doubleList.add(ages.get(i));
            doubleList.add(genders.get(i));
            doubleList.add(chols.get(i));
            doubleList.add(ntk.get(i));

            bps.set(i, doubleList);
        }
